...
class Battery:
    def __init__(self, sql_input, _id: int = 0, name: str = "None", percentage: int = 0, num_cycles: int = 0):
...
class BatteryDB:
    def __init__(self, db_file="batteries.db"):
        ...
        cursor = conn.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS batteries (id INTEGER PRIMARY KEY, name STRING, "
                       "percentage INTEGER NOT NULL, num_cycles INTEGER NOT NULL)")

        cursor.execute("""CREATE TABLE IF NOT EXISTS batteries_data (
    ...
    def add_battery_data(self, battery_data: BatteryData):
        ...
        current_battery = Battery(
            cursor.execute("SELECT * FROM batteries WHERE id=" + str(battery_id) + "").fetchall()[0])
